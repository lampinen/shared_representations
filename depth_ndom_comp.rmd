  ...
  for (nlayer in c(2, 3)) {
    for (ndomains in 1:3) {
      for (rseed in 0:(num_runs-1)) {
        filename = sprintf("./results/depth_and_ndom_comp_dontcares/%s_nlayer_%i_ndomains_%i_rseed_%i_loss_track.csv",network,nlayer,ndomains,rseed)
...
```
```{r}
ggsave('result_plots/depth_ndom_dontcares.png')
```
