...
\begin{itemize}
    ...
    \end{columns}
\end{itemize}
\uncover<5-> {Why does this occur?} %Can neural networks perform amortized inference about analogies?}
\end{frame}
...
So we need non-linear networks. Is there a way we can still use SVD analyses? \uncover<2->{\textbf{Yes}, with a single non-linearity, just analyze linear portion.}\\[11pt]
\only<1>{ \includegraphics[width=0.66\textwidth]{figures/network_with_IO.png}}
\only<2>{ \includegraphics[width=0.83\textwidth]{figures/network_with_IO_and_QM.png}}
