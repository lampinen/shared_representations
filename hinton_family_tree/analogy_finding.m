found_time = zeros(100,1);
found_at_settling = zeros(100,1);
found_flipped = zeros(100,1);
...
for run = 1:100
    ...
    single_l1_reps = [load(sprintf('%shinton_%snhidden_%i_eta_%f_%smomentum_0.000000_rseed_%i_f1_single_input_pre_middle_reps.csv',file_path,batch,nhidden,eta,init_type,run-1)); load(sprintf('%shinton_%snhidden_%i_eta_%f_%smomentum_0.000000_rseed_%i_f2_single_input_pre_middle_reps.csv',file_path,batch,nhidden,eta,init_type,run-1))];
...
% 
    ...
    if all(curr_assignment == intended_analogy) || all(curr_assignment == flipped_analogy)
         ...
         %found_time(run) = 0
         found_at_settling(run) = 1;
         if all(curr_assignment == flipped_analogy)
             found_flipped(run) = 1; 
         end
         %return
         continue
