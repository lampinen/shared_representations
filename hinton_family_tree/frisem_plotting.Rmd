...
```{r}
plotting_data = sl_input %>% gather(Analogy,num_projections,Regular,Flipped,Either,Null)
ggplot(plotting_data,aes(x=Analogy,y=num_projections,fill=Analogy)) +
  ...
  scale_x_discrete(limits=c('Regular','Flipped','Either','Null'),labels=c('Regular','Flipped','Either','Null Dist.')) +
  scale_fill_manual(limits=c('Null','Regular','Flipped','Either'),values=cbPalette)
ggsave('results/input_mode_projections_violin.png')
```


```{r}
plotting_data = sl_output %>% gather(Analogy,num_projections,Regular,Flipped,Either,Null)
ggplot(plotting_data,aes(x=Analogy,y=num_projections,fill=Analogy)) +
  geom_violin(adjust=2) +
  scale_y_continuous(breaks=0:12,limits=c(0,12)) +
  ylab("# significant modes for that analogy")+
...
```{r}
plotting_data = pfl_alt_input %>% gather(Analogy,num_projections,Regular,Flipped,Either,Null)
ggplot(plotting_data,aes(x=Analogy,y=num_projections,fill=Analogy)) +
  geom_violin(adjust=2) +
  scale_y_continuous(breaks=0:12,limits=c(0,12)) +
  ylab("# significant modes for that analogy")+
...
```{r}
plotting_data = pfl_anal_input %>% gather(Analogy,num_projections,Regular,Flipped,Either,Null)
ggplot(plotting_data,aes(x=Analogy,y=num_projections,fill=Analogy)) +
  geom_violin(adjust=2) +
  scale_y_continuous(breaks=0:12,limits=c(0,12)) +
  ylab("# significant modes for that analogy")+
